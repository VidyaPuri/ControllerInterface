...
class Joystick:
    ...
    def __init__(self,refreshRate = 30):
        self.proc = subprocess.Popen(['xboxdrv','--no-uinput','--detach-kernel-driver'], stdout=subprocess.PIPE, bufsize = 5, universal_newlines=True)
        ...
        if self.refreshTime < time.time():
            ...
            while readable:
                ...
                if len(response) == 140:
                    self.connectStatus = True
                    self.reading = response
                    #self.A =int(self.reading[100:101])
                    #print(self.A)
    ...
    # Left stick Y axis value scaled between -1.0 (left) and 1.0 (right) with deadzone tolerance correction
